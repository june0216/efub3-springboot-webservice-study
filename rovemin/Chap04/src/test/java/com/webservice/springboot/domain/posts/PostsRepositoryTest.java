...
public class PostsRepositoryTest {
    ...
    @AfterEach // Junit에서 단위 테스트가 끝날 때마다 수행되는 메소드 지정
    public void cleanup() {
    ...
    public void 게시글저장_불러오기() {
        ...
        postsRepository.save(Posts.builder()    // postsRepository.save: 테이블 posts에 insert/update 쿼리 실행.
                .title(title)                                   // id 값이 있다면 update, 없다면 insert 쿼리 실행
                .content(content)
                .author("rovemin@gmail.com")
                .build());

        //when
        List<Posts> postsList = postsRepository.findAll();      // postsRepository.findAll: 테이블 posts에 있는 모든 데이터 조회
